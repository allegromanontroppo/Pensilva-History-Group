...
const PersonTemplate: React.FC<PersonTemplateProps> = ({ data }: PersonTemplateProps) => {
  return (
    <OffCanvas>
      <Layout>
        ...
        <div className="row">
          <div className="column">
            <h1>
              People: <span className="subheader">{data.person.frontmatter.title}</span>
            </h1>
          </div>
        </div>
        <div className="row">
...
export const query = graphql`
  query($slug: String!) {
    person: markdownRemark(fields: { slug: { eq: $slug }, type: { eq: "people" } }) {
      html
      frontmatter {
        title
      }
